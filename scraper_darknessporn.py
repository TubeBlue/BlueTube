"""
Blue Tube - DarknessPorn Scraper
Scrapes video content from darknessporn.com
"""
...
class DarknessPornScraper:
    """Scraper for darknessporn.com website"""
    ...
    def scrape_homepage(self, max_videos: int = 10) -> List[Dict]:
        ...
        print(f"Scraping DarknessPorn homepage for up to {max_videos} videos...")
        ...
        try:
            ...
            if not video_elements:
                video_elements = soup.find_all('article', class_='post')
            ...
            for idx, element in enumerate(video_elements[:max_videos]):
                try:
                    ...
                    if video_data:
                        video_data['id'] = f"darknessporn_{idx + 1}"
                        video_data['source'] = 'darknessporn'
                        videos.append(video_data)
                        print(f"Scraped DarknessPorn video {idx + 1}: {video_data.get('title', 'Unknown')[:50]}")
                except Exception as e:
                    print(f"Error extracting DarknessPorn video {idx + 1}: {str(e)}")
                    continue
            
            print(f"Successfully scraped {len(videos)} videos from DarknessPorn")
            return videos
            
        except Exception as e:
            print(f"Error scraping DarknessPorn: {str(e)}")
            return []
    ...
    def _extract_video_data(self, element) -> Optional[Dict]:
        ...
        if video_id:
            ...
            # If no embed, use video page URL
            video_data['embed_url'] = video_data['video_url']
