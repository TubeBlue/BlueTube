"""
Blue Tube - DrTuber.com Scraper
Scrapes video content from drtuber.com
"""
...
class DrTuberScraper:
    """Scraper for drtuber.com website"""
    ...
    def scrape_homepage(self, max_videos: int = 500) -> List[Dict]:
        ...
        print(f"Scraping DrTuber.com for up to {max_videos} videos...")
        ...
        try:
            while len(videos) < max_videos:
                ...
                video_elements = soup.find_all('div', class_='th')
                ...
                for element in video_elements:
                    ...
                    try:
                        ...
                        if video_data:
                            video_data['id'] = f"drtuber_{len(videos) + 1}"
                            video_data['source'] = 'drtuber.com'
            ...
            print(f"\n✅ DrTuber: Scraped {len(videos)} videos from {page} pages")
            return videos
    ...
    def _extract_video_data(self, element) -> Optional[Dict]:
        ...
        # Get video link
        link = element.find('a', class_='th-img') or element.find('a')
        if link:
            ...
            # Extract video ID from URL - pattern: /video/{id}/
            match = re.search(r'/video/(\d+)', video_url)
            if match:
                video_id = match.group(1)
                video_data['video_id'] = video_id
                video_data['embed_url'] = f"https://www.drtuber.com/embed/{video_id}"
        ...
        if not video_data.get('title'):
            title_elem = element.find('div', class_='title') or element.find('h3')
            if title_elem:
        ...
        if img:
            thumbnail = img.get('data-src') or img.get('data-original') or img.get('src')
...
def main():
    """Test the scraper"""
    scraper = DrTuberScraper()
    videos = scraper.scrape_homepage(max_videos=300)
    
    if videos:
        print(f"\n✅ Total: {len(videos)} videos from DrTuber.com")
        with open('drtuber_videos.json', 'w', encoding='utf-8') as f:
            json.dump(videos, f, indent=2, ensure_ascii=False)
        print("Saved to drtuber_videos.json")
