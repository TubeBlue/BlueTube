"""
Blue Tube - HellPorno.com Scraper
Scrapes video content from hellporno.com
"""
...
class HellPornoScraper:
    """Scraper for hellporno.com website"""
    
    def __init__(self):
        self.base_url = "https://hellporno.com"
        self.headers = {
    ...
    def scrape_homepage(self, max_videos: int = 500) -> List[Dict]:
        ...
        print(f"Scraping HellPorno.com for up to {max_videos} videos...")
        ...
        try:
            while len(videos) < max_videos:
                ...
                for element in video_elements:
                    ...
                    try:
                        ...
                        if video_data:
                            video_data['id'] = f"hellporno_{len(videos) + 1}"
                            video_data['source'] = 'hellporno.com'
            ...
            print(f"\n✅ HellPorno: Scraped {len(videos)} videos from {page} pages")
            return videos
    ...
    def _extract_video_data(self, element) -> Optional[Dict]:
        ...
        link = element.find('a', class_='item-link') or element.find('a')
        if link:
            ...
            if match:
                ...
                video_data['embed_url'] = f"https://hellporno.com/embed/{video_id}/"
        ...
        # Get duration
        duration_elem = element.find('span', class_='duration') or element.find('div', class_='time')
        if duration_elem:
...
def main():
    """Test the scraper"""
    scraper = HellPornoScraper()
    videos = scraper.scrape_homepage(max_videos=300)
    
    if videos:
        print(f"\n✅ Total: {len(videos)} videos from HellPorno.com")
        with open('hellporno_videos.json', 'w', encoding='utf-8') as f:
            json.dump(videos, f, indent=2, ensure_ascii=False)
        print("Saved to hellporno_videos.json")
