"""
Blue Tube - YouPorn Scraper
Scrapes video content from youporn.com
"""
...
class YouPornScraper:
    """Scraper for youporn.com website"""
    ...
    def scrape_homepage(self, max_videos: int = 20) -> List[Dict]:
        ...
        print(f"Scraping YouPorn homepage for up to {max_videos} videos...")
        ...
        try:
            ...
            # Find video containers - YouPorn uses div with class 'video-box'
            video_elements = soup.find_all('div', class_='video-box')
            
            if not video_elements:
                video_elements = soup.find_all('div', class_='thumb-container')
            ...
            for idx, element in enumerate(video_elements[:max_videos]):
                try:
                    ...
                    if video_data:
                        video_data['id'] = f"youporn_{idx + 1}"
                        video_data['source'] = 'youporn'
                        videos.append(video_data)
                        print(f"Scraped YouPorn video {idx + 1}: {video_data.get('title', 'Unknown')[:50]}")
                except Exception as e:
                    print(f"Error extracting YouPorn video {idx + 1}: {str(e)}")
                    continue
            
            print(f"Successfully scraped {len(videos)} videos from YouPorn")
            return videos
            
        except Exception as e:
            print(f"Error scraping YouPorn: {str(e)}")
            return []
    ...
    def _extract_video_data(self, element) -> Optional[Dict]:
        ...
        # Find video link
        video_link = element.find('a', href=re.compile(r'/watch/'))
        
        if not video_link:
            return None
        
        # Get title from title attribute or data-title
        title = video_link.get('title', '').strip()
        if not title:
            title = video_link.get('data-title', '').strip()
        if not title:
            title_elem = video_link.find('div', class_='video-title')
            if title_elem:
        ...
        # Extract duration
        duration_elem = element.find('div', class_='video-duration')
        ...
        if video_id:
            ...
            # Get clean title for URL
            clean_title = re.sub(r'[^a-z0-9]+', '-', title.lower()).strip('-')
            video_data['embed_url'] = f"{self.base_url}/embed/{video_id}/{clean_title}/"
    ...
    def _extract_video_id(self, video_url: str) -> str:
        ...
        Example: https://www.youporn.com/watch/16989718/title -> 16989718
        """
        match = re.search(r'/watch/(\d+)/', video_url)
        if match:
