"""
Blue Tube - Fapality Scraper
Scrapes video content from fapality.com
"""
...
class FapalityScraper:
    """Scraper for fapality.com website"""
    
    def __init__(self):
        self.base_url = "https://fapality.com"
        self.headers = {
    ...
    def scrape_homepage(self, max_videos: int = 10) -> List[Dict]:
        ...
        print(f"Scraping Fapality homepage for up to {max_videos} videos...")
        ...
        try:
            ...
            for idx, element in enumerate(video_elements[:max_videos]):
                try:
                    ...
                    if video_data:
                        video_data['id'] = f"fapality_{idx + 1}"
                        video_data['source'] = 'fapality'
                        videos.append(video_data)
                        print(f"Scraped Fapality video {idx + 1}: {video_data.get('title', 'Unknown')[:50]}")
                except Exception as e:
                    print(f"Error extracting Fapality video {idx + 1}: {str(e)}")
                    continue
            
            print(f"Successfully scraped {len(videos)} videos from Fapality")
            return videos
            
        except Exception as e:
            print(f"Error scraping Fapality: {str(e)}")
            return []
    ...
    def _extract_video_data(self, element) -> Optional[Dict]:
        ...
        if not title:
            title_elem = video_link.find('h2') or video_link.find('h3')
            if title_elem:
